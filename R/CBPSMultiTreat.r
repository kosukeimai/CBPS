CBPS.3Treat<-function(treat, X, method, k, XprimeX.inv, bal.only, iterations, standardize, twostep, ...)
{
  probs.min<-1e-6
  ...
  gmm.func<-function(beta.curr,X.gmm=X,invV=NULL){
    ...
    gbar<-c(1/n*t(X)%*%(T2-probs.curr[,2]), 
            1/n*t(X)%*%(T3-probs.curr[,3]),
            w.curr.del)
  ...
  bal.loss<-function(beta.curr){
    ...
    loss1<-sum(diag(t(w.curr)%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr)))
    loss1
  }
  ...
  gmm.gradient<-function(beta.curr, X.gmm=X, invV)
  {
    ...
    dgbar<-rbind(cbind(1/n*t(-X*probs.curr[,2]*(1-probs.curr[,2]))%*%X,
                       1/n*t(X*probs.curr[,2]*probs.curr[,3])%*%X,
                       1/n*t(X*(2*T1*probs.curr[,2]/probs.curr[,1] + T2*(1-probs.curr[,2])/probs.curr[,2] - T3*probs.curr[,2]/probs.curr[,3]))%*%X,
                       1/n*t(X*(-T2*(1-probs.curr[,2])/probs.curr[,2] - T3*probs.curr[,2]/probs.curr[,3]))%*%X),
                 cbind(1/n*t(X*probs.curr[,2]*probs.curr[,3])%*%X,
                       1/n*t(-X*probs.curr[,3]*(1-probs.curr[,3]))%*%X,
                       1/n*t(X*(2*T1*probs.curr[,3]/probs.curr[,1] - T2*probs.curr[,3]/probs.curr[,2] + T3*(1-probs.curr[,3])/probs.curr[,3]))%*%X,
                       1/n*t(X*(T2*probs.curr[,3]/probs.curr[,2] + T3*(1-probs.curr[,3])/probs.curr[,3]))%*%X))
  ...
  bal.gradient<-function(beta.curr)
  {
    ...
    dw.beta1<-cbind(t(X*(2*T1*probs.curr[,2]/probs.curr[,1] + T2*(1-probs.curr[,2])/probs.curr[,2] - T3*probs.curr[,2]/probs.curr[,3])),
                    t(X*(-T2*(1-probs.curr[,2])/probs.curr[,2] - T3*probs.curr[,2]/probs.curr[,3])))/n
    dw.beta2<-cbind(t(X*(2*T1*probs.curr[,3]/probs.curr[,1] - T2*probs.curr[,3]/probs.curr[,2] + T3*(1-probs.curr[,3])/probs.curr[,3])),
                    t(X*(T2*probs.curr[,3]/probs.curr[,2] + T3*(1-probs.curr[,3])/probs.curr[,3])))/n
    ##Generate mean imbalance.
    loss1<-diag(t(w.curr)%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr))
    out.1<-2*dw.beta1[,1:n]%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr[,1]) + 
      2*dw.beta1[,(n+1):(2*n)]%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr[,2])
    out.2<-2*dw.beta2[,1:n]%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr[,1]) +
      2*dw.beta2[,(n+1):(2*n)]%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr[,2])
    out<-c(out.1, out.2)
    out
  }
...
CBPS.4Treat<-function(treat, X, method, k, XprimeX.inv, bal.only, iterations, standardize, twostep, ...)
{
  probs.min<-1e-6
  ...
  gmm.gradient<-function(beta.curr, X.gmm=X, invV)
  {
    ...
    dgbar<-rbind(cbind(1/n*t(-X*probs.curr[,2]*(1-probs.curr[,2]))%*%X,
                       1/n*t(X*probs.curr[,2]*probs.curr[,3])%*%X,
                       1/n*t(X*probs.curr[,2]*probs.curr[,4])%*%X,
                       1/n*t(X*(T1*probs.curr[,2]/probs.curr[,1] - T2*(1 - probs.curr[,2])/probs.curr[,2] - T3*probs.curr[,2]/probs.curr[,3] - T4*probs.curr[,2]/probs.curr[,4]))%*%X,
                       1/n*t(X*(T1*probs.curr[,2]/probs.curr[,1] + T2*(1 - probs.curr[,2])/probs.curr[,2] - T3*probs.curr[,2]/probs.curr[,3] + T4*probs.curr[,2]/probs.curr[,4]))%*%X,
                       1/n*t(X*(-T1*probs.curr[,2]/probs.curr[,1] - T2*(1 - probs.curr[,2])/probs.curr[,2] - T3*probs.curr[,2]/probs.curr[,3] + T4*probs.curr[,2]/probs.curr[,4]))%*%X
    ),
    cbind(1/n*t(X*probs.curr[,2]*probs.curr[,3])%*%X,
          1/n*t(-X*probs.curr[,3]*(1-probs.curr[,3]))%*%X,
          1/n*t(X*probs.curr[,3]*probs.curr[,4])%*%X,
          1/n*t(X*(T1*probs.curr[,3]/probs.curr[,1] + T2*probs.curr[,3]/probs.curr[,2] + T3*(1 - probs.curr[,3])/probs.curr[,3] - T4*probs.curr[,3]/probs.curr[,4]))%*%X,
          1/n*t(X*(T1*probs.curr[,3]/probs.curr[,1] - T2*probs.curr[,3]/probs.curr[,2] + T3*(1 - probs.curr[,3])/probs.curr[,3] + T4*probs.curr[,3]/probs.curr[,4]))%*%X,
          1/n*t(X*(-T1*probs.curr[,3]/probs.curr[,1] + T2*probs.curr[,3]/probs.curr[,2] + T3*(1 - probs.curr[,3])/probs.curr[,3] + T4*probs.curr[,3]/probs.curr[,4]))%*%X
    ),
    cbind(1/n*t(X*probs.curr[,2]*probs.curr[,4])%*%X,
          1/n*t(X*probs.curr[,3]*probs.curr[,4])%*%X,
          1/n*t(-X*probs.curr[,4]*(1-probs.curr[,4]))%*%X,
          1/n*t(X*(T1*probs.curr[,4]/probs.curr[,1] + T2*probs.curr[,4]/probs.curr[,2] - T3*probs.curr[,4]/probs.curr[,3] + T4*(1 - probs.curr[,4])/probs.curr[,4]))%*%X,
          1/n*t(X*(T1*probs.curr[,4]/probs.curr[,1] - T2*probs.curr[,4]/probs.curr[,2] - T3*probs.curr[,4]/probs.curr[,3] - T4*(1 - probs.curr[,4])/probs.curr[,4]))%*%X,
          1/n*t(X*(-T1*probs.curr[,4]/probs.curr[,1] + T2*probs.curr[,4]/probs.curr[,2] - T3*probs.curr[,4]/probs.curr[,3] - T4*(1 - probs.curr[,4])/probs.curr[,4]))%*%X
    ))
  ...
  bal.gradient<-function(beta.curr)
  {
    ...
    out.1<-2*dw.beta1[,1:n]%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr[,1]) +
      2*dw.beta1[,(n+1):(2*n)]%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr[,2]) +
      2*dw.beta1[,(2*n+1):(3*n)]%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr[,3])
    out.2<-2*dw.beta2[,1:n]%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr[,1]) +
      2*dw.beta2[,(n+1):(2*n)]%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr[,2]) +
      2*dw.beta2[,(2*n+1):(3*n)]%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr[,3])
    out.3<-2*dw.beta3[,1:n]%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr[,1]) +
      2*dw.beta3[,(n+1):(2*n)]%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr[,2]) +
      2*dw.beta3[,(2*n+1):(3*n)]%*%X%*%XprimeX.inv%*%t(X)%*%(w.curr[,3])
